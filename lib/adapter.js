...
var SqlContext = require('./context.js');
var utils = require('./utils');
var CursorJoin = require('waterline-cursor');

function logError(err) {
  if (typeof sails === "object" && typeof sails.log === "object" && typeof sails.log.error === "function") {
    sails.log.error(err);
  }
}
...
module.exports = (function () {
  ...
  var adapter = {
    ...
    find: function (connection, collection, options, cb) {
      ...
      var context = new SqlContext(connections[connection].collections[collection].definition);
      var statement = context.selectQuery(tableName, options);
    ...
    stream: function (connection, collection, options, stream) {
      ...
      if (options.groupBy || options.sum || options.average || options.min || options.max) {
        if (!options.sum && !options.average && !options.min && !options.max) {
          return stream.end(new Error('Cannot groupBy without a calculation'));
        }
      }
     ...
     * @param {object} data - Object with values for query parameters (parameterName, parameterValue)
     * @param {function} cb
     */
    query: function (connection, collection, query, data, cb) {

      // handle optional data argument
    ...
    create: function (connection, collection, values, cb) {
      ...
      var context = new SqlContext(connections[connection].collections[collection].definition);
      var statement = context.insertQuery(tableName, values);
      var parameters = context.getParameters();
    ...
    getPrimaryKey: function (connection, collection) {
      ...
      Object.keys(connections[connection].collections[collection].definition).forEach(function (key) {
        if (!connections[connection].collections[collection].definition[key].hasOwnProperty('primaryKey')) {
          return;
        }
        pk = key;
      });
    ...
    update: function (connection, collection, options, values, cb) {
      ...
      var context = new SqlContext(connections[connection].collections[collection].definition);
      var criteria = context.serializeOptions(collection, options);
      var parameters = context.getParameters();
      ...
      adapter.connectConnection(connection, function __UPDATE__(err, uniqId) {
        ...
        request.query(statement, function (err, recordset) {
          ...
          statement = 'UPDATE ' + tableName + ' SET ' + context.updateCriteria(collection, values) + ' ';
          statement += context.serializeOptions(collection, options);
          var parameters = context.getParameters();
